                ...
                {[
                  {
                    ...
                    valor: `${user.nombres} ${user.apellidos}`,
                  },
                  { label: "Correo", valor: user.correo },
                  {
                    label: "Teléfono",
                    valor: user.telefono || "No registrado",
                  },
                  {
                    label: "Edad",
                    valor: user.edad ? `${user.edad} años` : "No registrado",
                  },
                    ...
                    ? [
                        {
                          ...
                          valor: user.tipoColegio || "No registrado",
                        },
                      ]
                    : []),
                ].map((item, i) => (
            ...
            {user.carreraRecomendada ? (
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-12 text-center space-y-4">
                ...
                <h3 className="text-xl font-black text-slate-800">
                  Aún no has hecho el test
                </h3>
                <p className="text-slate-500">
                  Completa el test vocacional para descubrir qué carrera es
                  ideal para ti.
                </p>
                <Link
                  to="/test"
                  ...
                  {user.historialTests.map((t, i) => (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p
                            className={`font-bold text-sm ${i === 0 ? "text-blue-800" : "text-slate-800"}`}
                          >
                            {t.resultado}
                          </p>
            ...
            {user.universidadesFavoritas?.length > 0 ? (
              <>
                <p className="text-sm text-slate-400 mb-4">
                  {user.universidadesFavoritas.length} universidad
                  {user.universidadesFavoritas.length > 1 ? "es" : ""} guardada
                  {user.universidadesFavoritas.length > 1 ? "s" : ""}
                </p>
                <div className="space-y-3">
                  {user.universidadesFavoritas.map((uni, i) => (
                    <div
                      key={i}
                      className="flex items-center justify-between p-4 bg-slate-50 rounded-xl border border-slate-100 hover:bg-red-50 hover:border-red-100 transition-colors"
                    >
                      <div className="flex items-center gap-3">
              ...
              <div className="text-center py-12 text-slate-400 space-y-3">
                ...
                <p className="font-medium text-slate-500">
                  No tienes universidades favoritas aún.
                </p>
                <p className="text-sm">
                  Explora el directorio y guarda las que más te interesen.
                </p>
                <Link
                  to="/directorio"
                  className="inline-block bg-blue-600 hover:bg-blue-700 text-white font-bold px-6 py-2.5 rounded-xl transition text-sm mt-2"
              ...
              {[
                {
                  ...
                  valor: user.estudiantesAsignados || 0,
                  color: "purple",
                  emoji: "👥",
                },
                {
                ...
                {
                  label: "Test revisados",
                  valor: user.testRevisados || 0,
                  color: "amber",
                  emoji: "📋",
                },
                {
                  label: "Recursos compartidos",
                  valor: user.recursosCompartidos || 0,
                  color: "blue",
                  emoji: "📚",
                },
              ].map((stat, i) => (
                <div
                  key={i}
                  className="bg-white rounded-2xl shadow-sm border border-slate-100 p-5 text-center space-y-2"
                >
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 space-y-4">
            ...
            <div className="space-y-3">
              {[
                {
                  label: "Especialidad",
                  valor: user.especialidad || "Ingeniería de Sistemas",
                },
                {
                  label: "Grado académico",
                  valor: user.gradoAcademico || "Magíster en Educación",
                },
                {
                  label: "Años de experiencia",
                  valor: user.aniosExperiencia || "—",
                },
                {
                  label: "Estado de cuenta",
                  valor: user.estadoCuenta || "Activo",
                },
