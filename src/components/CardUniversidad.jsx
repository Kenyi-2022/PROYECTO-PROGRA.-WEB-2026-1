...
function CardUniversidad({ id, logo, nombre, tipo, carreras, escalas, webOficial, ubicacion, costoMatricula, busqueda = "", isDark = false, isAdmin = false, onEliminarUni, onEliminarCarrera }) {
  const { user, marcarFavoritoContext, favoritos } = useApp();
  const navigate = useNavigate();
  const texto = busqueda.toLowerCase().trim();
const esFavorito = favoritos.some(f => f.universidadId === id);
  //const esFavorito = user?.favoritos?.includes(id) || false;
