...
function CardUniversidad({ logo, nombre, tipo, carreras, escalas, webOficial, ubicacion, costoMatricula, busqueda = "", isDark = false }) {
  ...
  return (
    // CONTENEDOR PRINCIPAL DINÁMICO (Dark/Light Mode)
    <div className={`w-full rounded-2xl shadow-md hover:shadow-xl transition-all duration-300 border overflow-hidden ${isDark ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-100'}`}>
      ...
      <div className="flex items-center gap-3 px-6 pb-4 flex-wrap">
        <button onClick={() => setMostrarCarreras(!mostrarCarreras)} className="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2 rounded-lg text-sm font-bold transition">
          {mostrarCarreras ? "Ocultar carreras" : `Ver ${carreras.length} carreras`}
        </button>

        {escalas && escalas.length > 0 && (
          <button onClick={() => setMostrarEscalas(!mostrarEscalas)} className={`${isDark ? 'bg-slate-800 text-slate-300 hover:bg-slate-700' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'} px-5 py-2 rounded-lg text-sm font-bold transition`}>
        ...
        {esFavorito && user && (
          <button onClick={irAFavoritos} className="text-red-500 text-xs font-medium hover:underline ml-auto">Ver mis favoritos →</button>
        )}
      </div>
            ...
            return (
              <div key={index} className={`flex items-start justify-between gap-4 px-6 py-4 border-b last:border-0 transition-colors ${coincide ? (isDark ? "bg-indigo-900/30" : "bg-yellow-50") : (isDark ? "hover:bg-slate-950 border-slate-800" : "hover:bg-slate-50 border-slate-50")}`}>
                ...
                {carrera.planEstudios && (
                  <a href={carrera.planEstudios} target="_blank" rel="noopener noreferrer" className={`shrink-0 text-xs font-bold px-3 py-1.5 rounded-lg transition border flex items-center gap-1 ${isDark ? 'bg-indigo-500/10 text-indigo-400 border-indigo-500/20 hover:bg-indigo-500/20' : 'bg-indigo-50 text-indigo-700 border-indigo-100 hover:bg-indigo-100'}`}>
                    📄 Malla
                  </a>
                )}
              </div>
